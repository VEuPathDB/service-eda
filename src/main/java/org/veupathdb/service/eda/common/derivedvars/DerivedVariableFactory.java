...
import org.veupathdb.service.eda.common.derivedvars.plugin.AbstractDerivedVariable;
...
public class DerivedVariableFactory {
  ...
  private void addDerivedVariableInstances(List<DerivedVariableSpec> derivedVariableSpecs, List<DerivedVariable> allInstanceList) {
    for (DerivedVariableSpec spec : derivedVariableSpecs) {
      ...
      boolean found = addDerivedVariableInstance(spec, Transform.getPlugins(), _transforms, allInstanceList);
      if (found) continue;
      // check name against reductions
      found = addDerivedVariableInstance(spec, Reduction.getPlugins(), _reductions, allInstanceList);
      if (!found)
