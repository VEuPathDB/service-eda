...
@JsonDeserialize(
    as = BinWidthSpecImpl.class
)
public interface BinWidthSpec {
  @JsonProperty("type")
  TypeType getType();
  ...
  interface RangeType {
    ...
    class RangeDeserializer extends StdDeserializer<RangeType> {
      ...
      public RangeType deserialize(JsonParser jsonParser, DeserializationContext jsonContext) throws
        ...
        if ( looksLikeNumberRange(map) ) return new BinWidthSpecImpl.RangeTypeImpl(mapper.convertValue(map, NumberRangeImpl.class));
        if ( looksLikeDateRange(map) ) return new BinWidthSpecImpl.RangeTypeImpl(mapper.convertValue(map, DateRangeImpl.class));
