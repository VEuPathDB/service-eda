...
public class ServiceExternal implements Merging {
  ...
  public static List<DerivedVariableMetadata> processDvMetadataRequest(DerivedVariableBulkMetadataRequest requestBody) {
    try {
      List<String> requestDvColumns = VariableDef.toDotNotation(requestBody.getDerivedVariables());
      return new RequestResourcesBase(requestBody)
