...
/**
 * Parent class providing logic and interface common to all Transform derived
 * variables, that is: a derived variable that operates on data in a single row
 * to produce a single value in the same entity.  Instances of this class
 * represent a derived variable with a certain configuration which will be
 * generated across an entire set of rows.  Thus, the getValue() method's
 * implementation should be stateless/pure so its results are reproducible
...
public abstract class Transform<T> extends AbstractDerivedVariable<T> {
  ...
  /**
   * Primary data implementation method which produces a single value from a row
   * of values.  The incoming row should NOT be manipulated; it is only for
   * pulling out values needed to calculate this derived value.
   *
  ...
  public final void validateDependedVariableLocations() {
    // find the ancestors of the entity this var is declared on; dependant vars
    // must live on the same entity as this var or an ancestor
